...
AssetManager.prototype.asset = function (identifier, options) {
    ...
    //TODO: Strip duplicates from the filenames list
    ...
    for (var filename, i = 0, len = filenames.length; i < len; i++) {
        ...
        if (!(filenames[i] in this.assets)) {
            ...
            var message = format('Asset "%s" could not be found', filename);
            this.emit('error', new Error(message));
            return '';
        }
    ...
    var hash = utils.hashString(assetHashes, this.hash);
    return format('asset-%s-%s', hash, identifier);
};
