...
Manager.prototype.init = function (app) {
    ...
    var helperName = this.options.viewHelper
      ...
      , pathHelper = this.assetPath.bind(this)
      , self = this;
    ...
    app.use(function (request, response, next) {
        ...
        response.locals[helperName].path = pathHelper;
        next();
    });
    app.use(this.options.servePrefix, function (request, response, next) {
        ...
        if (!(canonical in self.compilingNow)) {
            ...
            self.compileAsset(asset, function (err) {
                self.compilingNow[canonical].forEach(function (next) {
                    next(err);
                });
